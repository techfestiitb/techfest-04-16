...
$title = "Techfest Web Team File Manager";// Title (may contain HTML tags)
$windowtitle = $title;             // Window title (text only)
$defaultstatusmsg = "Techfest Team";        // Default status message (text only)

$bodybgcolor  = "#5398F3";         // Background color of page body
...
$warningcolor = "#FF8000";         // Warning messages forground color
$errorcolor   = "#FF0000";         // Error messages forground color
...
$allowemptytrashcan = false;        // Allowing to empty trash can
...
$allowdelete = true;               // Allowing file deletion
$allowremovedir = true;            // Allowing directory deletion
...
$filemode = 0644;                  // Create mode for files (UNIX)
$dirmode = 0755;                   // Create mode for directories (UNIX)
$showunixattrs = true;            // Show perms / owner / group (UNIX)
...
$builtinauth = true;              // Use builtin authentication (needs PHP to be running as module)
$authuser = "techfest";                // Builtin authentication user
$authpassword = "techteam";        // Builtin authentication password
$realmname = "Web File Browser";   // Realm name

$useimages = true;                // Use images, set to false by default to respect the philosophy
                                   // "all in one single file" (but its much nicer with)
$imagesdir = "";          // Images directory (must be located in base directory)
$showimagesdir = true;            // Show images directory
$trashcanimage = "images/trashcan.gif";   // Image for trash can
$upperdirimage = "images/upperdir.gif";   // Image for upper and main directory
$opendirimage = "images/opendir.gif";     // Image for open directory
$dirimage = "images/dir.gif";             // Image for simple directory
$fileimage = "images/file.gif";           // Image for file directory
$editimage ="images/edit.gif";           // Image for edit action
$viewimage = "images/view.gif";           // Image for view action
$downloadimage = "images/download.gif";   // Image for download action

$allowsearch = true;               // Allow searches
$searchmaxlevels = 1;             // Search levels (max depth in sub directories for searches, 0 means no limits)
...
// *** I INSIST *** : be carefull to what you do here !
// Lot of people ask me questions because of their mis-usage of these parameters...
...
$messages = array(
   ...
   "mov0"=>"Move",
   "mov1"=>"Bad name for file(s) or folder(s) to move",
   "mov2"=>"Bad destination folder for file(s) or folder(s) to move",
   ...
   "rmd0"=>"Remove",
   "rmd1"=>"Bad name for folder to remove",
...
function addFileToList($file, $fp, $alias, $level, $image = "", $msg = "") {
   global $files, $subdir, $trashcan, $sortby, $showunixattrs, $dateformat, $useimages, $imagesdir, $dirimage,
$fileimage, $messages;
...
function pageHeader() {
   ...
   echo "<img src='images/techlogo.jpg' height='100' width ='100'>";
         ...
         while ($file = @readdir($d)) {
            if (checkFileName($file)) {
               ...
               if ($filealiases && @is_readable($fp_alias)) {
                  $fd = @fopen($fp_alias, "r");
                  $alias = trim(@fread($fd, @filesize($fp_alias)))." <i>(".(($subdir ==
$trashcan) ? ereg_replace("(.*)\.[0-9]*$", "\\1", $file) : $file).")</i>";
                  @fclose($fd);
               }
      ...
      if ($searchpattern != "") {
         ...
         if (searchFiles($subdir, $searchpattern) == 0) {
            redirectWithMsg("warning", "sch3", $searchpattern, "",
"&searchpattern=".rawurlencode($searchpattern).(($allowregexpsearch) ? "&regexpsearch=$regexpsearch" : ""));
         }
      ...
      if (isset($HTTP_POST_FILES["file"]) && ($HTTP_POST_FILES["file"]["size"] > 0)) {
         if (!checkFileName($HTTP_POST_FILES["file"]["name"]) || (($subdir == "") && ($HTTP_POST_FILES["file"]["name"]
== $trashcan))) {
...
if ($allowsearch && ($subdir != $trashcan) && (($act == "") || ($act == "search"))) {
   ...
   echo "<input type=button value=\"".$messages["sch2"]."\" onClick='submitActForm(document.searchForm,
\"searchpattern\", \"".$messages["sch6"]."\")'>";
   if ($allowregexpsearch) {
      echo "<br><input type=checkbox value=true name=regexpsearch".(($regexpsearch) ? " checked" : "").">
".$messages["sch7"];
   }
   echo "</td>";
}
   ...
   if (isset($file) && ($file != "")) {
      if (!checkFileName($file) || (($subdir == "") && ($file == $trashcan))) {
         ...
         if ($act == "edit") {
            ...
            echo "<tr>";
            echo "<td align=left>";
            echo "<input type=radio name=fileformat value=dos".(($defaultfileformat ==
"dos") ? " checked" : "").">".$messages["sav6"];
            echo "<br><input type=radio name=fileformat value=unix".(($defaultfileformat ==
"unix") ? " checked" : "").">".$messages["sav7"];
            echo "</td>";
            ...
            echo "</table>";
            echo "<p><a href=$thisscript?subdir=".rawurlencode($subdir)."&sortby=$sortby onMouseOver='return
statusMsg(\"".$messages["edt12"]."\");' onMouseOut='return statusMsg(\"\");'>".$messages["edt12"]."</a>";
         }
      }
   } else {
...
// File list page
   ...
   if ($act != "search") {
      ...
      echo    "\nalert(\"".$messages["act6"]."\");";
      echo "\n} else if (((action == 'delete') || (action == 'rmdir')) && confirm(\"".$messages["act2"]." ?\")) {";
      echo    "\nf.act.value = action;";
      echo    "\nf.submit();";
   ...
   if (!empty($files)) {
      ...
      echo "<th>";
      echo "<a href=$thisscript?subdir=".rawurlencode($subdir)."&sortby=name".(($act ==
"search") ? "&act=search&searchpattern=".rawurlencode($searchpattern) : "")." onMouseOver='return
statusMsg(\"".$messages["inf1"]."\");' onMouseOut='return statusMsg(\"\");'>".$messages["tab3"]."</a>";
      echo "</th>";
      echo "<th>";
      echo "<a href=$thisscript?subdir=".rawurlencode($subdir)."&sortby=size".(($act ==
"search") ? "&act=search&searchpattern=".rawurlencode($searchpattern) : "")." onMouseOver='return
statusMsg(\"".$messages["inf2"]."\");' onMouseOut='return statusMsg(\"\");'>".$messages["tab4"]."</a>";
      echo "</th>";
      echo "<th>";
      echo "<a href=$thisscript?subdir=".rawurlencode($subdir)."&sortby=date".(($act ==
"search") ? "&act=search&searchpattern=".rawurlencode($searchpattern) : "")." onMouseOver='return
statusMsg(\"".$messages["inf3"]."\");' onMouseOut='return statusMsg(\"\");'>".$messages["tab5"]."</a>";
      echo "</th>";
...
onMouseOver='return statusMsg(\"".$file["statusmsg"]."\");' onMouseOut='return statusMsg(\"\");'>";
                  echo "<b>".$file["alias"]."</b>";
                  echo "</a>";
               }
               echo "</td>";
         ...
         // File section
            ...
            if ($filelinks) {
               ...
               echo " onMouseOver='return statusMsg(\"".$file["statusmsg"]."\");' onMouseOut='return
statusMsg(\"\");'>";
               echo $file["alias"];
               echo "</a>";
            } else {
            ...
            echo "<td align=center>&nbsp;";
            if (($act != "search") && ($allowedit || $allowshow) && checkExtension($file["name"]) && ($subdir !=
$trashcan)) {
               if (!$file["readonly"] && $allowedit) {
                  echo "<a
href=$thisscript?act=edit&subdir=".rawurlencode($subdir)."&sortby=$sortby&file=".rawurlencode($file["name"])." onMouseOver='return
statusMsg(\"".$messages["edt8"]."\");' onMouseOut='return statusMsg(\"\");'>".(($useimages) ? "<img
src=\"$imagesdir/$editimage\" border=0>" : $messages["edt10"])."</a> ";
               }
               echo "<a
href=$thisscript?act=show&subdir=".rawurlencode($subdir)."&sortby=$sortby&file=".rawurlencode($file["name"])." onMouseOver='return
statusMsg(\"".$messages["edt9"]."\");' onMouseOut='return statusMsg(\"\");'>".(($useimages) ? "<img
src=\"$imagesdir/$viewimage\" border=0>" : $messages["edt11"])."</a> ";
            } 
            if (($allowdownload) && ($subdir != $trashcan)) {
               echo "<a
href=$thisscript?act=download&subdir=".rawurlencode($subdir)."&sortby=$sortby&file=".str_replace("%2F", "/",
rawurlencode(extractSubdir($file["path"])))." onMouseOver='return statusMsg(\"".$messages["dwn5"]."\");'
onMouseOut='return statusMsg(\"\");'>".(($useimages) ? "<img
src=\"$imagesdir/$downloadimage\" border=0>" : $messages["dwn1"])."</a> ";
            }
            echo "</td>";
            echo "</tr>";
      ...
      echo "<th colspan=$nbcols>$nbdirs ".$messages["tab11"].", $nbfiles
".$messages["tab12"]." (".round($total/1024)." ".$messages["tab13"].")</th>";
      echo "</tr>";
      ...
      if ($subdir != $trashcan) {
         ...
         if (   ($act != "search")
            ...
            echo "<input type=button value=\"".$messages["mkd0"]."\" onClick='submitActForm(document.createDirForm,
\"file\", \"".$messages["mkd4"]."\")'>";
            echo "</td>";
            echo "</tr>";
            echo "</form>";
         ...
         if (   ($act != "search")
            ...
            echo "<td class=tdlt colspan=".($nbcols - 3).">";
            echo "<input name=file type=text size=15> ";
            echo "<input type=button value=\"".$messages["cre0"]."\" onClick='submitActForm(document.createFileForm,
\"file\", \"".$messages["cre4"]."\")'>";
            echo "</td>";
            echo "</tr>";
            echo "</form>";
         ...
         if (   ($act != "search")
            ...
            echo "<input type=button value=\"".$messages["fup0"]."\" onClick='submitActForm(document.uploadFileForm,
\"file\", \"".$messages["fup4"]."\")'>";
            echo "</td>";
            echo "</tr>";
            echo "</form>";
         ...
         if (   ($act != "search")
            ...
            echo "<input type=button value=\"".$messages["uup0"]."\" onClick='submitActForm(document.uploadUrlForm,
\"file\", \"".$messages["uup4"]."\")'>";
            echo "</td>";
            echo "</tr>";
            echo "</form>";
