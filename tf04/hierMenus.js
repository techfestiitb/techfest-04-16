...
//3.10 modified next statement
//	clickKill = NSfontOver = keepHilite = clickStart = false;
...
//3.10 omitted next statement to enable font color change for UNIX
...
function startIt() {
	...
	if (clickKill) {
		...
		oldclicked = menuLoc.document.onmousedown;
		menuLoc.document.onmousedown = clicked;
	}
	makeTop();
	status = 'Techfest 2004';
}
...
function makeElement(whichEl,whichWidth,whichParent,whichContainer) {
	if (NS4) {
		...
		if (IE5) {
			...
			elStr = "<DIV ID=" + whichEl + " STYLE='position:absolute;z-index:999;'></DIV>";
...
/*3.10 omitted next 6 statements and moved the 7th below
...
*/
//3.10 added the following 11 statements to create 2 item text layers
...
//3.10 omitted next 4 lines; replaced with 2 lines above
//		if (NSfontOver) {
...
//	else parent.frames.midFrame.location.href = this.linkText;
}
