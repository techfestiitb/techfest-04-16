...
*/
Hi="5.2";
if(HM_Konqueror)HM_IE=false;HM_IE5M=HM_IE&&HM_Mac;HM_NS6=(navigator.vendor==("Netscape6")||navigator.product==("Gecko"));HM_IE5W=HM_IE&&!HM_Mac;HM_IEpos=!HM_NS6||(parseInt(navigator.productSub)>=20010710);
...
Hac="HM_Item";Had="HM_Array";Function.prototype.gk=true;Function.prototype.gm=false;String.prototype.gk=false;String.prototype.gm=true;String.prototype.gn=false;String.prototype.gl=false;Number.prototype.gm=false;Number.prototype.gk=false;
...
jp.siblingBelow=null;jp.hg=null;jp.cs=null;jp.Fs=null;jp.Ft=null;jp.onmouseenter=null;jp.onmouseleave=null;jp.onmouseover=null;jp.onmouseout=null;jp.onclick=null;jp.onmousedown=null;jp.onselectstart=null;jp.onmousewheel=null;jp=null;}Hjn[i]=null;
...
Hjs=(HM_IE&&Hg.document.doctype)?Hg.document.doctype.name.indexOf(".dtd")!=-1:Hjr;Hjt=(HM_IE)&&!Hjs;Hjh=(typeof(Hg.document.body.onmouseenter)!="undefined");if(HM_IE||HM_Konqueror)Hh=Hjr?Hg.document.documentElement:Hg.document.body;
if(Hk3){HXw=(Hg.document.onmousedown)?Hg.document.onmousedown:HXD;Hg.document.onmousedown=HXC;}else{Hbw=HL*1000;}var iu=HM_IE?Hg.document.body:Hg;HXx=(iu.onresize)?iu.onresize:HXD;iu.onresize=HWN;if(H4){HXy=(iu.onunload)?iu.onunload:HXD;iu.onunload=HWO;
...
HW5,false);Hji=true;}else if(parent.HM_UseFrameLoad){Hji=true;parent.HM_f_LoadMenus=HW5;}Hjm=true;if(k9){var iu=HM_IE?Hg.document.body:Hg;iu.onunload=HWO;HXy=HXD;Hg.location.replace(Hg.location.href);return false;}}}}else{var k9=HXF();
...
function HWU(){var jw=false;var jx=null;try{var jy=typeof(Hg.document);if((HM_IE)&&(typeof(Hg.document)=="unknown"))return false;if((HM_IE)&&(Hg.document.readyState!="complete"))return false;jx=((Hg)&&(Hg.document))?Hg.document:null;
...
function HWG(ix){var gw=Hg.document.createElement("DIV");HWR(gw);var k=((Hjt)?Hbf.k:Hbf.cc)+"px";with(gw){id=ix;with(style){position="absolute";visibility="hidden";left="-500px";top="-2000px";width=k;
...
Hkr.onmouseleave=HWg;}else{Hkr.onmouseover=HWf;Hkr.onmouseout=HWg;}Hkr.HXd=HWo;Hkr.HXg=HWr;Hkr.HXh=HWs;Hkr.HXe=HWp;Hkr.HXf=HWq;Hkr.bm=false;Hkr.isOn=false;Hkr.bc=null;Hkr.bn=null;Hkr.HXo=HWH;Hkr.hn=false;Hkr.HXq=HWJ;Hkr.bq=false;Hkr.HXj=HWv;Hkr.HXk=HWw;
Hkr.HXl=HWA;Hkr.hd=false;if(HM_IE)Hkr.onselectstart=HWL;Hkr.moveTo=HWK;Hkr.HXo();if(HM_IE5M&&HV&&!Hkr.cw&&!(Hbf.ch&&Hkr.ct==Hbf.cm)&&!(Hbf.ce&&Hkr==Hbf.cm)){Hkr.HXj();}while(Hkr.bo<Hkr.cy){Hkr.bo++;Hkr.gx=Hg.document.getElementById(Hac+cq+"_"+Hkr.bo);
if(!Hkr.gx){if(HN)window.status="Creating Hierarchical Menus:"+cq+"/"+Hkr.bo;Hkr.gx=HWW(cq);}if(!Hlg&&Hkr.gx.dc&&(!HS||Hkr.cv)){dd=HWc(cq+"_"+Hkr.bo);if(dd){Hkr=Hkr.ct;}}}Hg.document.body.appendChild(Hkr);if(!HM_IE5M)Hkr.HXq();return Hkr;}
...
this.menu.style.width=hj+((Hjt)?(Hu*2):(parseInt(paddingLeft)+parseInt(paddingRight)))+"px";}}else{left="0px";if(!this.menu.cz){if(Hjt)width=Hbf.cc+"px";else width=(Hbf.cc-(parseInt(paddingLeft)+parseInt(paddingRight)))+"px";}}}}
...
el.style.left=(el.index?parseInt(el.siblingBelow.style.left)+parseInt(el.siblingBelow.style.width):0)+"px";}else{if(el.hasImage&&!Hbf.R){if(!Hjr)el.hh+=Ht;}el.hh-=(parseInt(el.style.paddingLeft)+parseInt(el.style.paddingRight));
if(!HM_NS6&&!Hjr&&!HM_IE5M&&!el.hi)el.hh-=Hw;el.ki=el.tree.cc-(parseInt(el.style.paddingLeft)+parseInt(el.style.paddingRight));el.style.width=Math.min(el.ki,el.hh)+"px";
...
this.style.width=this.cs.style.width=hj+((Hjt)?(Hu*2):(parseInt(el.style.paddingLeft)+parseInt(el.style.paddingRight)))+"px";}}}var hk=0;for(var i=0;i<ek;i++){el=Items[i];if(el.index){var dt=el.siblingBelow.offsetHeight-((Hjt)?0:(Ht*2));hk=Math.max(hk,dt);
}else{hk=el.offsetHeight-((Hjt)?0:(Ht*2));}}for(var i=0;i<ek;i++){el=Items[i];el.style.height=hk+"px";if(el.hg){if(this.tree.R){el.hg.style.left=(Ht+HD)+"px";
}else{el.hg.style.left=(el.offsetWidth-((el.hi?0:Hw)+(Hjt?HM_IE5M?Ht:Ht*2:Hjr?Ht*2:Ht)+HD+HC))+"px";}}}this.style.height=this.cs.style.height=hk+((Hjt)?Hu*2:(Ht*2))+"px";}else{if(this.cz){var hl=0;for(var i=0;i<ek;i++){el=Items[i];
el.hh=(HM_IE)?el.scrollWidth:el.offsetWidth;if(isNaN(el.hh))el.hh=el.offsetWidth;if(HM_IE5M)el.hh+=(parseInt(el.style.paddingLeft)+parseInt(el.style.paddingRight));hl=Math.min((Math.max(hl,el.hh)),this.tree.cc);}for(var i=0;i<ek;
i++){if(Hjt){Items[i].style.width=hl+"px";}else{Items[i].style.width=(hl-parseInt(Items[i].style.paddingRight)-parseInt(Items[i].style.paddingLeft))+"px";}}this.style.width=this.cs.style.width=(Items[0].offsetWidth+((Hjt)?Hu*2:0))+"px";}for(var i=0;i<ek;
i++){var el=Items[i];if(el.index){var dt=(el.siblingBelow.offsetHeight);el.style.top=parseInt(el.siblingBelow.style.top)+dt+"px";}else el.style.top="0px";if(el.hg){if(this.tree.R){el.hg.style.left=(Ht+HD)+"px";
}else{el.hg.style.left=(el.offsetWidth-((Hjt?HM_IE5M?Ht:Ht*2:Hjr?Ht*2:Ht)+HD+HC))+"px";}}}this.style.height=this.cs.style.height=parseInt(el.style.top)+(HM_IE5W?el.scrollHeight:el.offsetHeight)+((Hjt)?(Hu*2):0)+"px";}this.hm=this.style.height;
if(HM_IE5M&&this.bq){with(this.Fs.style){width=(parseInt(this.style.width)-(Hjt?(Hu*2):0))+"px";height=(Hk2-(Hjt?0:(Hu*2)))+"px";}with(this.Fs.firstChild.style){top=((Hk2-(Hu*2)-H1)/2)+"px";left=((parseInt(this.Fs.style.width)-Hit)/2)+"px";
}with(this.Ft.style){width=(parseInt(this.style.width)-(Hjt?(Hu*2):0))+"px";height=(Hk2-(Hjt?0:(Hu*2)))+"px";}with(this.Ft.firstChild.style){top=((Hk2-(Hu*2)-H1)/2)+"px";left=((parseInt(this.Ft.style.width)-Hit)/2)+"px";}}this.hn=true;
if(iz)this.style.visibility="visible";}
...
function HWZ(){if(Hay||Hjl||Hax||!HWU())return;Hjl=true;if(!this.child){Hbf=this.tree;Hkr=this.menu;Hkr.gx=this;this.child=HWc(this.menu.count+"_"+(this.index+1));Hbf=this.tree;}Hjl=false;if(this.menu.style.visibility=="hidden")return;
...
var Fa=this.xPos;var Fb=Fa+this.offsetWidth;if((this.yPos<ev)&&(this.cv||this.tree.lf))this.yPos=ev;var Fc=this.yPos+parseInt(this.hm);Fc+=(Hjt)?0:(Hu*2);if(this.cv){var Fd=this.cu.oL;
}if((Fb>ey)&&(this.cv||this.tree.le)){if(this.cv){this.xPos=((Fd-this.cu.dx)+this.cu.I)-this.offsetWidth;}else{var dif=Fb-ey;this.xPos-=dif;}this.xPos=Math.max(0,this.xPos);}if((Fc>ez)&&(this.cv||this.tree.lf)){var dif=Fc-ez;this.yPos-=dif;
}if((Fa<eu)&&(this.cv||this.tree.le)){if(this.cv){this.xPos=(Fd+this.cu.dw)-this.cu.I;Fb=this.xPos+this.offsetWidth;if(Fb>ey)this.xPos-=(Fb-ey);}else{this.xPos=0}}if(HV){if(((this.yPos<ev)&&(this.cv||this.tree.lf))||(Fc>ez)){var kl=(Hjt)?0:(Hu*2);
if(this.yPos<ev){if(!H4||H5!="top"){this.yPos=(ev+et);var Fj=(et*2);}else{this.yPos=ev;var Fj=et;}this.style.height=(Math.max(ex,((Hk2*2)+H3))-Fj-kl)+"px";}else{this.style.height=(Math.max((ez-this.yPos),((Hk2*2)+H3))-kl)+"px";}if(!this.bq)this.HXj();
this.HXl();}else if(this.bq){if(!HM_IE5W){if(parseInt(this.hm)<ex)this.style.height=this.hm;}this.HXl();}}}
...
function HWv(){var Fm=Hg.document.createElement("DIV");with(Fm.style){position="absolute";top=-(Hu)+"px";width=(parseInt(this.style.width)-((Hjt)?(Hu*2):0))+"px";height=(Hk2-((Hjt)?0:(Hu*2)))+"px";visibility="hidden";backgroundColor=Hk1;textAlign="center";
zIndex=10000;var km=(Hu+"px "+this.tree.v+" "+Hhf);borderBottom=km;borderTop=km;}var kh=Hg.document.createElement("IMG");with(kh.style){position="absolute";top=((Hk2-(Hu*2)-H1)/2)+"px";visibility="inherit";left=((parseInt(Fm.style.width)-Hit)/2)+"px";
}Fm.appendChild(kh);var kn=Fm.cloneNode(true);if(Hky)kn.firstChild.src=Hky;if(Hkz)Fm.firstChild.src=Hkz;this.appendChild(Fm);this.appendChild(kn);Fm.menu=this;this.Fs=Fm;kn.menu=this;this.Ft=kn;if(Hkw){if(Hjh){this.Fs.onmouseenter=HXA;
this.Fs.onmouseleave=HWx;this.Ft.onmouseenter=HXB;this.Ft.onmouseleave=HWx;}else{this.Fs.onmouseover=HXA;this.Fs.onmouseout=HWx;this.Ft.onmouseover=HXB;this.Ft.onmouseout=HWx;}}else{this.Fs.onmousedown=HXA;this.Ft.onmousedown=HXB;}this.bq=true;}
